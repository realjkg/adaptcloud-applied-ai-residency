...
| Sandbox | required first | same container with sandbox profile | validate and plan | manual apply/destroy allowed |
...
Create GitHub environments named `sandbox`, `staging`, and `production`. Require a reviewer for all three; restrict staging and production to the default branch. Put only non-secret identifiers in environment variables. Cloud trust must restrict the OIDC subject to the student's exact fork and environment.
...
9. Rerun with operation `destroy` and confirmation `DESTROY MY SANDBOX`; verify the state and billing console.
10. Produce staging and production plans as architecture evidence. Apply them only from a separate organization-controlled promotion system after the required production controls exist.
